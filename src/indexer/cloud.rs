...
pub async fn insert_bundle_entry(block_id: u32, bundle_txid: &str, envelope_txid: &str) -> Result<(), Error> {
...
pub async fn get_latest_block_id() -> Result<u32, Error> {
   let conn = ps_client().await?;
   let query_str = "SELECT MAX(block_id) as max_block FROM bundles_indexer";
